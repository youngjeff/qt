...
#include <QDebug>
frmInput *frmInput::_instance = 0;   //实例对象
...
void frmInput::Init(QString position, QString style, int btnFontSize, int labFontSize)//初始化面板状态,包括字体大小
{
    ...
    this->btnFontSize = btnFontSize; //按钮字体
    this->labFontSize = labFontSize; //标签字体
    this->ChangeStyle();             //改变样式
    this->ChangeFont();              //改变字体
}
...
void frmInput::mouseMoveEvent(QMouseEvent *e)
{
    ...
    {
        this->move(e->globalPos() - mousePoint);//e->globalPos() - this->pos(); 通过这种方式可以一直移动
...
void frmInput::InitForm()            //初始化窗体数据
{
    ...
    this->setWindowFlags(Qt::Tool | Qt::WindowStaysOnTopHint | Qt::FramelessWindowHint);//窗口置顶
    QDesktopWidget w;
    deskWidth = w.availableGeometry().width();//桌面实际可用大小
    ...
    currentStyle = "";              //当前输入法面板样式

    //输入法面板字体大小,如果需要更改面板字体大小,该这里即可
    btnFontSize = 10;               //当前输入法面板按钮字体大小
    labFontSize = 10;               //当前输入法面板标签字体大小

    //如果需要更改输入法初始模式,改变currentType这个变量即可
    //min--小写模式  max--大写模式  chinese--中文模式
    currentType = "min";            //当前输入法类型
    changeType(currentType);        //改变输入法类型
...
void frmInput::InitProperty()
{
    ...
    labCh.append(ui->labCh0);        //将标签依次插入汉字标签数组
...
void frmInput::reClicked()//reclicked是处理按键的
{
    if (isPress)
    {
        timerPress->setInterval(30);//设置每次按键的时间间隔
        btnPress->click();
    }
}
...
void frmInput::selectChinese()
{
    clearChinese();
    QSqlQuery query;
    QString currentPY = ui->labPY->text();//获取要查询的单词
    ...
    while(query.next())
    {
        ...
        QStringList text = result.split(" ");//字符串链表,将字串变成字符数组
...
void frmInput::showChinese()
{
    ...
    for (int i = currentPY_index; i < currentPY_count; i++)
    {
        ...
        QString txt = QString("%1.%2").arg(count).arg(allPY[currentPY_index]);//arg 可以将数字,字串合并为一个
...
void frmInput::btn_clicked()//处理控件用的
{
    ...
    QPushButton *btn = (QPushButton *)sender();//btn 就是发送信号的键
    ...
    } else if (objectName == "btnPre")
    {
        ...
        showChinese();//show 的时候从currentPY_index 小到大
    }
...
void frmInput::clearChinese()
{
    ...
    allPY.clear();                   //所有拼音链表
    currentPY.clear();               //当前拼音链表
    currentPY_index = 0;             //当前拼音索引
    currentPY_count = 0;             //当前拼音数量
}
